...
for n = -N:N
    subplot(4,2,1)
    stem(k,x,'MarkerFaceColor','blue'), axis([-N N -1.5 ceil(L/2)+1]), xlabel('k'), ylabel('x[k]')
    subplot(4,2,2)
    ...
    if n<0
        ...
        stem(k,x,'MarkerFaceColor','blue'), axis([-N N -1.5 ceil(L/2)+1]), xlabel('k'), ylabel(sprintf('x[k], h[%i-k]',n))
        hold off
        subplot(4,2,[5,6])
        stem(k,x.*[h(end-abs(n):-1:1) zeros(1,abs(n))],'k','MarkerFaceColor','black'), axis([-N N -1.5 ceil(L/2)+1])
        ...
        axis([-N N -1 8]), xlabel('n'), ylabel('y[n] = x[n]\ast h[n]')
    else
        subplot(4,2,[3,4])
        ...
        hold off 
        subplot(4,2,[5,6])
        stem(k,x.*[zeros(1,n) h(end:-1:1+n)],'k','MarkerFaceColor','black'), axis([-N N -1.5 ceil(L/2)+1])
        xlabel('k'), ylabel(sprintf('x[k]h[%i-k]',n))
        subplot(4,2,[7,8])
