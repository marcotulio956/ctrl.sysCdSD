...
for n = -N:N
    ...
    if n<0
        ...
        hold off
        subplot(4,2,[5,6])
        stem(k,x.*[h(end-abs(n):-1:1) zeros(1,abs(n))],'k','MarkerFaceColor','black'), axis([-N N -2 4])
        ...
        axis([-N N -1 ceil(L/2)^2+1]), xlabel('n'), ylabel('y[n] = x[n]\ast h[n]')
    else
        subplot(4,2,[3,4])
        ...
        hold off 
        subplot(4,2,[5,6])
        stem(k,x.*[zeros(1,n) h(end:-1:1+n)],'k','MarkerFaceColor','black'), axis([-N N -2 4])
        xlabel('k'), ylabel(sprintf('x[k]h[%i-k]',n))
        subplot(4,2,[7,8])
