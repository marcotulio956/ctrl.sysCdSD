...
for n = -N:N
    ...
    if n<0
        if mod(n,1/dt) == 0
            ...
            hold off
            subplot(4,2,[5,6])
            plot(t,x.*[h(end-abs(n):-1:1) zeros(1,abs(n))],'k','MarkerFaceColor','black'), axis([-T T -2 4])
        ...
        if mod(n,1/dt) == 0
            ...
            axis([-T T -1 ceil(L/2)^2+1]), xlabel('t'), ylabel('y(t) = x(t)\ast h(t)')
        end
    else
        if mod(n,1/dt) == 0
            subplot(4,2,[3,4])
            ...
            hold off
            subplot(4,2,[5,6])
            plot(t,x.*[zeros(1,n) h(end:-1:1+n)],'k','MarkerFaceColor','black'), axis([-T T -2 4])
            xlabel('\tau'), ylabel(['x(\tau)h(' sprintf('%2.0f',n*dt) '-\tau)'])
            subplot(4,2,[7,8])
        end
